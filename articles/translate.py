...
def translate_tokens(native_tokens_u, native_language, target_language = 'en'):
  if not isinstance(target_language, basestring):
    target_language = target_language.code
  if not isinstance(native_language, basestring):
    native_language = native_language.code
  translations_u_u = {}
  for sentence_u in chunk(TRANSLATE_CHUNK_SIZE, (native_tokens_u)):
    target_text_u = service.translations().list(source = native_language,
                                target = target_language,
                                q = list(sentence_u)).execute()
    target_text_u = map(lambda target_token: target_token['translatedText'], target_text_u['translations'])
    for native_token_u, target_token_u in izip(sentence_u, target_text_u):
      translations_u_u[native_token_u] = target_token_u
  return translations_u_u
